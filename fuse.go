...
//
package fuse // import "github.com/simonchen/fuse"
...
var bufSize = maxRequestSize + maxWrite * 10

func printParams() {
	fmt.Println("maxRequestSize = ", maxRequestSize)
...
loop:
	...
	n, err := syscall.Read(c.fd(), m.buf)
        //fmt.Println("[Simon] fuse.Conn.Request - syscall.Read n = ", n)
	c.rio.RUnlock()
	...
	m.buf = m.buf[:n] 
	//fmt.Println("[Simon] fuse.Conn.Request m.buf has length = " , m.len())
	...
	case opRead, opReaddir:
		...
		//fmt.Println("[Simon] fuse.Conn.Request:opRead, opReaddir: in.Offset = %d, in.Size = %d", in.Offset, in.Size)
		r := &ReadRequest{
			Header: m.Header(),
